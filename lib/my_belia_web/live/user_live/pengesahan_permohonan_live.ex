defmodule MyBeliaWeb.UserLive.PengesahanPermohonanLive do
  ...
  def handle_event("submit-application", _params, socket) do
    ...
    # Process staged documents if needed
    processed_documents = process_staged_documents(form_data["supporting_documents"] || %{})

    # Flatten to array of file identifiers (filenames) for storage
    supporting_paths = extract_document_paths(processed_documents)

    # Merge into attrs with user_id
    updated_form_data =
      form_data
      |> Map.put("supporting_documents", supporting_paths)
      |> Map.put("user_id", current_user && current_user.id)

    case GrantApplications.create_grant_application(updated_form_data) do
      {:ok, _grant_app} ->
  ...
  # Extract a flat list of identifiers from the processed documents map
  defp extract_document_paths(processed_documents) when is_map(processed_documents) do
    processed_documents
    |> Map.values()
    |> Enum.flat_map(fn
      nil -> []
      %{"filename" => name} -> [name]
      list when is_list(list) -> Enum.flat_map(list, fn
        %{"filename" => name} -> [name]
        _ -> []
      end)
      _ -> []
