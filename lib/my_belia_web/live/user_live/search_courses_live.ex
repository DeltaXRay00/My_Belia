defmodule MyBeliaWeb.UserLive.SearchCoursesLive do
  use MyBeliaWeb, :live_view

  def mount(_params, session, socket) do
    user_id = session["user_id"]
    current_user = if user_id, do: MyBelia.Accounts.get_user!(user_id)

    {:ok, assign(socket, current_user: current_user, query: "", courses: [], page_title: "Carian Kursus"), layout: false}
  end

  def render(assigns) do
    MyBeliaWeb.PageHTML.search_courses(assigns)
  end

  def handle_event("search", %{"query" => query}, socket) do
    if String.length(query) >= 2 do
      courses = MyBelia.Courses.search_courses(query)
      {:noreply, assign(socket, query: query, courses: courses)}
    else
      {:noreply, assign(socket, query: "", courses: [])}
    end
  end

  def handle_event("clear_search", _params, socket) do
    {:noreply, assign(socket, query: "", courses: [], loading: false)}
  end
end
